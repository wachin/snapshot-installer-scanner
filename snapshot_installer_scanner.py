...
class ScannerWorker(QObject):
    ...
    def __init__(self, db_path: str, root_path: str, label: str):
        super().__init__()
        self.db_path = db_path
    ...
    def run(self):
        ...
        try:
            ...
            snapshot_id = db.create_snapshot(self.label, self.root_path)
                        ...
                        for entry in it:
                            ...
                            try:
                                ...
                                if now - last_emit >= 0.2:
                                    self.progress.emit(0, f"Escaneados: {scanned:,} | Actual: {path}")
                                    last_emit = now
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        cfg_grid.addWidget(QLabel("Ruta a escanear:"), 0, 0)
    ...
    def _connect_signals(self):
        self.btn_scan_before.clicked.connect(lambda: self.start_scan("ANTES"))
    ...
    def start_scan(self, label: str):
        ...
        if not root:
            QMessageBox.warning(self, "Falta ruta", "Debes indicar una ruta a escanear.")
            return
        if not os.path.exists(root):
            QMessageBox.warning(self, "Ruta inválida", "La ruta a escanear no existe.")
            return
        ...
        self.append_log(f"\n=== Iniciando escaneo '{label}' de {root} ===")

        self.worker_thread = QThread()
        self.worker = ScannerWorker(db_path, root, label)
    ...
    def compare_snapshots(self):
        ...
        with open(summary_txt, "w", encoding="utf-8") as f:
            f.write(f"Comparación de snapshots\n")
